...
function CheckForCommands(user,message){ //Vérifie si le message est une commande
    ...
    {		Log("["+message.author.username+"] : "+message.content);
		...
		if(texte.substr(1,3) == "say"){ //Commande SAY
			...
			message.channel.send("- Statut du Nan0 : "+canKickNano);
			return true;
		}
		...
		else if(texte.substr(1,6) == "switch"){ //Commande SWITCH CHANNEL
			message.delete();
		...
		else if(texte.substr(1,2) == "yt")
		{
            ...
            if (user.role != 2) {
                message.channel.send("<@" + user.id + "> Chuuuut !");
                return true;
            }
        ...
        else if (texte.substr(1, 6) == "create")
        {
            bot.channels.get('304342040425267201').clone("Nan0 c 1 " + texte.substr(7, texte.length).toUpperCase(), true, true, "Parce que j'ai le pouvoir de le faire");
            message.channel.send("Channel créé")
        }
...
class BotUser {
	...
	changeRole(role)
	{
		...
		console.log(this.nom+"a maintenant un role de niveau : "+this.role)
	}
...
function IDOfMember(id)
{
	for(var i = 0; i < users.length; i++) {
		
		if(users[i].id == id)
			console.log("c'est "+user[i].name+" qui parle")
			return true;
	}
	return false;}
...

{
     ...
     message += " "+date.getHours()+"h"+date.getMinutes()+"min"+date.getSeconds()+"s";
...
function OptionsLancement()
{
	...
	data=fs.readFileSync('commandes.txt');
	data=data.toString();
	data=data.split(";");

	for (var i =0; i<data.length-1; i++) {
    	console.log("Commande "+data[i]);
    	Log("FileCommand:"+data[i]);
	CheckForConsoleCommand("#"+data[i])
	

		
	}
	//console.log("Contenu du fichier : "+data[0]);
...
var users = [new BotUser("Nadeko", '116275390695079945',0), new BotUser("Nan0", '95605730538684416',0), new BotUser("Koala", '155779054837039104',2), new BotUser("Maxime", '155772975554101248',2),
			new BotUser("Dodo", '221995947377426433',1), new BotUser("Anais", '234279599196995585',1), new BotUser("Delt0s", '108950296507486208',2), new BotUser("BOT","327044138434297866",0),
...
texte+="\n";
//console.log(texte);
fs.appendFile('log.txt', texte, function (err) {
  if (err) {
    // append failed
  } else {
    // done
...
bot.on('ready', () => {
    bot.user.setStatus("online", "with my code");
...
bot.on('typingStart', (channel, member) => {
	...
	if(nombretg==1)
	{
		...
		console.log("Nombre de ta gueule: "+nombretg)
	}

	if(nombretg==2)
	{
		channel.send("Non mais sérieux ferme ta gueule");
		console.log("Nombre de ta gueule: "+nombretg)
	}

	if(nombretg==3)
	{
		channel.send("Tu crois que je plaisante?");
		console.log("Nombre de ta gueule: "+nombretg)
	}
	...
	if(nombretg==4)
		...
		{
			channel.send("je t'avais prévenu!!!");
			setTimeout(function(){channel.send("DEMANDE MOI PARDON");}, 20000);
		}
	/*if(nombretg>=5)
	{
		channel.send("DEMANDE MOI PARDON");
	}*/
	}
});
//---------- Lorsqu'un message est envoyé sur le discord ----------
bot.on("voiceLeave", function (user, voiceChannel) {
    console.log(user + " left " + voiceChannel);
});
...
bot.on('message', (message) => {
   // bot.user.setGame('censurer le Nan0');
	...
	if(user === null) { //Si l'utilisateur n'est pas identifié
		console.log('Received message from ' +message.author.username + ' with ID : ' +message.author.id);
		 users[users.length]=new BotUser("tmp"+usertmp, message.author.id.toString(),0);
		 user=users[users.length-1];
        		
        		users[users.length-1].nbfois=0;
 ...
 //Il est temps d'agir
		...
		if(user.nom.substr(0,3)=="tmp") //si c'est un membre temporaire
        		{
        			//console.log(user)
        			if(user.nbfois==0)	//si c'est son premier message
        			{message.channel.send("Qu'est ce que tu fais là? <@" + message.author.id +"> je ne te connais pas, merci de te présenter aux adminitstrateurs : <@155779054837039104> ou encore <@155772975554101248>");
        			message.channel.send("Je me souviendrai provisoirement de toi en tant que "+message.author.username+" mais tu n'auras aucun droit sur ce serveur, même si c'est <@95605730538684416> qui t'a invité, en effet, je suis le fruit de plusieurs cinquantaines d'heures de travail pour purifier ce serveur, ce n'est pas une connaissance de nan0 qui mettrai tout en l'air");}
        			else
        			{message.delete();
        			message.channel.send("Désolé <@"+message.author.id+"> mais tu es un membre temporaire, tu n'as donc pas le droit de poster des messages");}
        			user.nbfois++;
        		}



		else if(user.nom == "Nadeko") {
			message.delete();
			...
			if(message.content.includes("maintenance")||(message.content.includes("mise à jour")&&message.content.includes("bot")))
			{
				message.channel.send("Ok, je m'éteins, mais tkt <@95605730538684416> je reviendrai plus fort");
				setTimeout(function(){
				bot.destroy();
				process.exit(0);}, 1000);
			...
			DoesLOLNeedTOBeInstalled(message);
        		...
        		else if (user.id == "327044138434297866")
 		{
 		}
		else if(user.nom == "Nan0") { //----------------------------------------------------------------------------------------------------------------------------------------------------
			...
			if(nombretg>=5)
			{
				if(message.content.includes("pardon")||message.content.includes("excuse")||message.content.includes("désolé"))
				{
					nombretg = 0;
					channel.send("Merci chien, mais arrête de parler");
				}
			...
			if(IsBotBG(message.content) && !user.firstMessage()) { //Le bot est-il un BG ?
     		...
     		else if (user.nom == "Nan0bot")
     		{
        		}
        		else if(user.nom="MossyFrog")
        		{
...
function CheckForConsoleCommand (texte){ //Vérifie si le texte entré dans la console est une commande
	...
	if(texte[0]!="#")
	Log("ConsoleCommand: "+texte)
	else
		texte=texte.substr(1,texte.length)
	...
	if(texte.substr(0,3) == "say"){ //Commande SAY
		...
		else if (texte == "commands" || texte == "help" || texte == "h")
		{
		...
		else if(texte == "debug")
		{
			...
			console.log("- Mute du nan0: "+canKickNano);
			console.log("--------------------------------");
			return;
		}
		else if(texte.substr(0,9) == "greetings")
		{
		...
		else if(texte.substr(0,6) == "switch")
		{
			texte = texte.substr(7,texte.length-1);
			var nextChannel = GetChannelFromName(texte);
			if(nextChannel == null) {
		...
		else if(texte.substr(0,4) == "vsay"||texte.substr(0,4)=="ysay") {
		texte = texte.substr(5,texte.length-1);
		Speak(VoiceChan, texte);}
	...
	else if(texte.substr(0,7) == "censure")
	{
		...
		console.log("Censure du nan0: "+censure);
        	...
        	else if (texte.substr(0, 4) == "lien") 
      	{
            lien = !lien;
            console.log("Censure des liens du nan0: "+lien);
        	}

        else if (texte.substr(0, 4) == "code")
        {
            curChannel.getChannel().send(texte.substr(5, texte.length), [false, "python"]);
        }

        else if (texte.substr(0, 6) == "create")
        {
            bot.channels.get('304342040425267201').clone("Nan0 c 1 " + texte.substr(7, texte.length).toUpperCase(), true, true, "Parce que j'ai le pouvoir de le faire");
        }


        else if (texte.substr(0,4) == "user")
        {
        	if(texte=="user")
        	{
        		console.log("======================")
		...
		console.log("=======================")
        		...
        		texte = texte.substr(5,texte.length-1);
		str=texte.split(" ");
		console.log("User demandé: "+str[0]);

        		tmp_user = GetUserFromName(str[0]);
        	try{
        		tmp_user.changeRole(str[1]);
        	}
        	catch(e)
        ...
        else if(texte.substr(0,3) == "tts")
        {

        	texteADire = texte.substr(4, texte.length-1);

        	fs.writeFile("tmp_tts/file.txt", texteADire, function(err) {
    if(err) {
    ...
    console.log("The file was saved!");
        	...
        	cmd.get('Powershell.exe -executionpolicy remotesigned -File  tts.ps1');

        	texte="../tmp_tts/0"
        	setTimeout(function(){Speak(VoiceChan, texte)}, 1000);
        	//Speak(VoiceChan, texte);
